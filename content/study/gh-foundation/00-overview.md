---
title: "GitHub Foundation"
date: 2024-08-08
draft: true
categories:
    - 'study'
---

# Learning - GitHub Foundation

Resources
[7 domains](https://assets.ctfassets.net/wfutmusr1t3h/1kmMx7AwI4qH8yIZgOmQlP/79e6ff1dfdee589d84a24dd763b1eef7/github-foundations-exam-study-guide__1_.pdf)
[26 notes](https://github.com/Andresmup/github-foundations/tree/main/00_THE_BASICS) 
[Practice tests](https://ghcertified.com/practice_tests/)

* Domain 1: Introduction to Git and GitHub (8 notes)
    * Git and GitHub Basics
    * GitHub Entities
    * GitHub Markdown
    * GitHub Desktop
    * GitHub Mobile
* Domain 2: Working with GitHub Repositories (1 note)
* Domain 3: Collaboration Features (8 notes)
    * Issues
    * Pull requests
    * Discussions
    * Notifications
    * Gist, Wikis, and GitHub Pages
* Domain 4: Modern Development (3 notes)
    * GitHub Actions
    * GitHub Copilot
    * GitHub Codespaces
* Domain 5: Project Management (1 note)
* Domain 6: Privacy, Security, and Administration (3 notes)
    * Authentication and Security
    * GitHub Administration
* Domain 7: Benefits of the GitHub Community
    * Describe the benefits of the open source community
    * Describe how to apply the benefits of open source